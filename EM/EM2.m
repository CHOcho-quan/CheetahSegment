...
for C = Cs
    ...
    BG_m(num2str(C)) = mu_grass;
    BG_s(num2str(C)) = sigma_grass;
    BG_p(num2str(C)) = pi_grass;
    FG_m(num2str(C)) = mu_cheetah;
    FG_s(num2str(C)) = sigma_cheetah;
    FG_p(num2str(C)) = pi_cheetah;
end
toc
...
for C = Cs
    for di = 1:size(dim, 2)
        res = zeros(rows, cols);
        d = dim(di);
        mu_BG = BG_m(num2str(C));
        mu_BG = mu_BG(:, 1:d);
        mu_FG = FG_m(num2str(C));
        mu_FG = mu_FG(:, 1:d);
        sigma_BG = BG_s(num2str(C));
        sigma_BG = sigma_BG(1:d, 1:d, :);
        sigma_FG = FG_s(num2str(C));
        sigma_FG = sigma_FG(1:d, 1:d, :);
        pi_BG = BG_p(num2str(C));
        pi_FG = FG_p(num2str(C));
        ...
        for c = 1:C
            inv_BG(:, :, c) = inv(sigma_BG(:, :, c));
            inv_FG(:, :, c) = inv(sigma_FG(:, :, c));
